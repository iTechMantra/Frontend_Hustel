// src/pages/AshaSignup.jsx
import { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { sendOTP, verifyOTP } from '../services/otpService';
import { registerAsha, setCurrentSession } from '../services/authService';
import { translate } from '../services/translationService';

export default function AshaSignup() {
  ...
  const handleSendOTP = async (e) => {
    ...
    if (!formData.aadhaar.trim()) { setError('Aadhaar number is required'); return; }
  ...
  const handleVerifyOTP = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        const registerResult = await registerAsha({ 
          name: formData.name, 
        ...
        if (registerResult.success) {
          await setCurrentSession({ current: { role: 'asha', uuid: registerResult.uuid, name: formData.name, phone: formData.phone, loggedAt: new Date().toISOString() } });
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-b from-purple-50 to-purple-100 font-sans">
      ...
      <main className="flex-grow flex items-center justify-center px-4">
        <div className="bg-white p-8 rounded-2xl shadow-xl w_full max-w-md border border-purple-100">
          <div className="text-center mb-6">
            <h2 className="text-2xl font-bold text-gray-800 mb-2">
              {translate('ASHA Registration')}
            </h2>
            <p className="text-gray-600">
              {translate('Join E-Sannidhi as an ASHA worker')}
            </p>
          </div>
            ...
            <form onSubmit={handleSendOTP} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  required
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                  placeholder={translate('Enter your location or village')}
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  maxLength="12"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                  placeholder={translate('Enter Aadhaar number')}
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  minLength="4"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                  placeholder={translate('Enter a 4+ digit PIN')}
                />
              </div>
            ...
            <form onSubmit={handleVerifyOTP} className="space-y-4">
              ...
              <div className="flex space-x-3">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="flex-1 bg-purple-600 text-white py-2 rounded-lg hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                >

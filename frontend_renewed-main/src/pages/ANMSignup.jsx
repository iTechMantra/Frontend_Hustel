...
export default function ANMSignup() {
  const navigate = useNavigate();
  const [step, setStep] = useState('form');
  const [form, setForm] = useState({ name: '', phone: '', subcentre: '' });
  ...
  const send = async (e) => {
    e.preventDefault();
    if (!form.name || !form.phone || !form.subcentre) { setError('All required fields'); return; }
  ...
  const verify = async (e) => {
    ...
    try {
      ...
      if (r.success) {
        await setCurrentSession({ current: { role: 'anm', uuid: form.phone, name: form.name, phone: form.phone, loggedAt: new Date().toISOString() } });
    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-b from-pink-50 to-pink-100 font-sans">
      <main className="flex-grow flex items-center justify-center px-4">
        <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md border border-pink-100">
          <h2 className="text-2xl font-bold text-gray-800 mb-4 text-center">{translate('ANM Signup')}</h2>
          ...
          {step==='form' ? (
            <form onSubmit={send} className="space-y-4">
              ...
              <div><label className="block text-sm font-medium text-gray-700 mb-1">{translate('Sub-centre Name')} *</label><input name="subcentre" value={form.subcentre} onChange={onChange} className="w-full px-3 py-2 border rounded" required /></div>
              <button type="submit" disabled={loading} className="w-full bg-pink-600 text-white py-2 rounded hover:bg-pink-700">{loading?translate('Sending...'):translate('Send OTP')}</button>
            </form>
          ) : (
            <form onSubmit={verify} className="space-y-4">
              ...
              <button type="submit" disabled={loading} className="w-full bg-pink-600 text-white py-2 rounded hover:bg-pink-700">{loading?translate('Verifying...'):translate('Verify')}</button>
            </form>
          )}
        </div>
      </main>

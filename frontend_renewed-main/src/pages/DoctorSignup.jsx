...
export default function DoctorSignup() {
  ...
  const handleSendOTP = async (e) => {
    ...
    if (!formData.specialization.trim()) { setError('Specialization is required'); return; }
  ...
  const handleVerifyOTP = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        const registerResult = await registerDoctor({ 
          name: formData.name, 
        ...
        if (registerResult.success) {
          await setCurrentSession({ current: { role: 'doctor', uuid: registerResult.uuid, name: formData.name, phone: formData.phone, loggedAt: new Date().toISOString() } });
  ...
  const specializations = [
    'General Medicine','Cardiology','Dermatology','Pediatrics','Gynecology','Orthopedics','Neurology','Psychiatry','Ophthalmology','ENT','Radiology','Anesthesiology','Emergency Medicine','Internal Medicine','Surgery'
  ];

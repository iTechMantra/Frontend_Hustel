...
import { verifyPIN, setOrUpdatePin, loginWithOTP, verifyOTPAndSetupSession, setCurrentSession } from '../services/authService';
import { translate } from '../services/translationService';

export default function AshaLogin() {
  const navigate = useNavigate();
  const [method, setMethod] = useState('password'); // 'password'(PIN) or 'otp'
  ...
  const handlePasswordLogin = async (e) => {
    ...
    try {
      // Treat password as PIN verification
      const pinResult = await verifyPIN(password, phone);
      if (pinResult.success) {
        // If session already exists, navigate; else create minimal session using phone
        await setCurrentSession({ current: { role: 'asha', uuid: phone, name: 'ASHA Worker', phone, loggedAt: new Date().toISOString() } });
  ...
  const handleOTPLogin = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        await verifyOTPAndSetupSession({ phone, name: 'ASHA Worker' });
        setSuccess('Login successful!');
        setTimeout(() => {
          navigate('/asha/dashboard');
        }, 800);

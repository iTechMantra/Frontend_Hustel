...
export default function UserLogin() {
  ...
  const handlePasswordLogin = async (e) => {
    ...
    if (!password.trim()) { setError('PIN is required'); return; }

    setLoading(true);
    setError('');
    setSuccess('');
    try {
      // Import and use the new loginUser function
  ...
  const handleOTPLogin = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        // Use the new verifyOTPAndSetupSession with correct role
        const { verifyOTPAndSetupSession } = await import('../services/authService');
        await verifyOTPAndSetupSession({ phone, name: 'Community Member', role: 'user' });
        setSuccess('Login successful!');
        setTimeout(() => navigate('/user/dashboard'), 800);
      } else {
            ...
            <form onSubmit={handleOTPLogin} className="space-y-4">
              ...
              {success && (
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full bg_green-600 text-white py-2 rounded-lg hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                >

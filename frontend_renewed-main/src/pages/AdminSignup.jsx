...
export default function AdminSignup() {
  ...
  const send = async (e) => {
    e.preventDefault();
    if (!form.name || !form.phone) { setError('All required fields'); return; }
  ...
  const verify = async (e) => {
    ...
    try {
      ...
      if (r.success) {
        await setCurrentSession({ current: { role: 'admin', uuid: form.phone, name: form.name, phone: form.phone, loggedAt: new Date().toISOString() } });
    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-b from-gray-50 to-gray-100 font-sans">
      <main className="flex-grow flex items-center justify-center px-4">
        <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md border border-gray-200">
          <h2 className="text-2xl font-bold text-gray-800 mb-4 text-center">{translate('Admin Signup')}</h2>
          ...
          {step==='form' ? (
            <form onSubmit={send} className="space-y-4">
              ...
              <button type="submit" disabled={loading} className="w-full bg-gray-700 text-white py-2 rounded hover:bg-gray-800">{loading?translate('Sending...'):translate('Send OTP')}</button>
            </form>
          ) : (
            <form onSubmit={verify} className="space-y-4">
              ...
              <button type="submit" disabled={loading} className="w-full bg-gray-700 text-white py-2 rounded hover:bg-gray-800">{loading?translate('Verifying...'):translate('Verify')}</button>
            </form>
          )}
        </div>
      </main>
